...
if ( ! class_exists( 'Generate_Products_XML' ) ) :
	/**
	 * Class definition.
	 */
	abstract class Generate_Products_XML {
		/**
			...
			foreach ( $products as $product ) {
				...
				if ( ! empty( $item ) ) {
					...
					if ( $item['additional_image'] ) {
						foreach ( $item['additional_image'] as $image_url ) {
							$item_node->addChild( 'g:additional_image_link', $image_url, 'http://base.google.com/ns/1.0' );
						}
					}
				}
		...
		private function get_formated_single_store_product( $product, $tag ) {
			...
			if ( $this->is_valid_product( $product ) ) {
				...
				$data['image_link']   = wp_get_attachment_image_src(
					$product->get_image_id(),
					$image_size ? 'virtuaria_facebook_image_size' : 'full'
				)[0];
				...
				if ( $product->is_type( 'variable' ) ) {
					...
					$data['price'] = number_format( floatval( $product->get_price() ), 2, ',', '.' );
				...
				if ( $product->get_gallery_image_ids() ) {
					foreach ( $product->get_gallery_image_ids() as $image_id ) {
						$data['additional_image'][] = wp_get_attachment_image_src(
							$image_id,
							$image_size ? 'virtuaria_facebook_image_size' : 'full'
						)[0];
					}
